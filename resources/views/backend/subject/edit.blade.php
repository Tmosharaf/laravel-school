            ...
            <form action="{{ route('subject.update', $subject) }}" method="post">
                ...
                <label class="block text-sm mb-4">
                    ...
                    <input
                        ...
                        value="{{ $subject->subject_name }}" name="subject_name" placeholder="Subject Name">
                    {{-- Errors --}}
                    @error('subject_name')
                ...
                <label class="block text-sm mb-4">
                    ...
                    <input
                        ...
                        value="{{ $subject->subject_code }}" name="subject_code" placeholder="Subject code">
                    {{-- Errors --}}
                    @error('subject_code')
                        <span class="text-xs text-red-600 dark:text-red-400">
                            {{ $message }}
                ...
                <label class="block text-sm mb-4">
                    ...
                    <select
                        ...
                        @foreach ($classes as $class)
                            <option value="{{ $class->id }}" {{ ($class->id == $subject->classes_id) ? 'selected' : '' }} >{{ $class->name }}</option>
                        @endforeach
                    </select>
